...
function convertUTCToIrishTime(utcHour, utcMinute, tideDateUTC) {
  ...
  return { hour: localDate.getHours(), minute: localDate.getMinutes(), timeStr: localDate.getHours().toString().padStart(2,'0')+":"+localDate.getMinutes().toString().padStart(2,'0') };
}

function getMoonIcon(phaseName) {
  const icons = { "New Moon": "🌑", "Waxing Crescent": "🌒", "First Quarter": "🌓", "Waxing Gibbous": "🌔", "Full Moon": "🌕", "Waning Gibbous": "🌖", "Last Quarter": "🌗", "Waning Crescent": "🌘" };
...
function getSelectedTime() { return currentHour.toString().padStart(2,'0')+":"+currentMinute.toString().padStart(2,'0'); }
...
function saveUserPreferences() {
  ...
  localStorage.setItem('divesense_preferences', JSON.stringify(prefs));
}
...
async function fetchRealTideData(station, date) {
  ...
  try {
    ...
    if (data.error || !data.extremes || data.extremes.length === 0) return { events: [], moonPhase: getMoonPhase(date), tideType: "Unknown", error: true };
    ...
    const selectedYear = date.getFullYear(), selectedMonth = date.getMonth(), selectedDay = date.getDate();
    const start = new Date(Date.UTC(selectedYear, selectedMonth, selectedDay, 0, 0, 0));
    const end = new Date(Date.UTC(selectedYear, selectedMonth, selectedDay, 23, 59, 59));
    let tideEvents = [];
    for (let extreme of data.extremes) {
      const tideUTC = new Date(extreme.dt * 1000);
      if (tideUTC >= start && tideUTC <= end) {
        const local = convertUTCToIrishTime(tideUTC.getUTCHours(), tideUTC.getUTCMinutes(), tideUTC);
        tideEvents.push({ type: extreme.type === "High" ? "High" : "Low", time: local.timeStr, height: extreme.height, timestamp: extreme.dt * 1000 });
      }
    }
    if (tideEvents.length === 0 && data.extremes.length) {
      for (let extreme of data.extremes) {
        const tideUTC = new Date(extreme.dt * 1000);
        const local = convertUTCToIrishTime(tideUTC.getUTCHours(), tideUTC.getUTCMinutes(), tideUTC);
        const localDate = new Date(Date.UTC(selectedYear, selectedMonth, selectedDay, local.hour, local.minute, 0));
        if (localDate.getUTCFullYear() === selectedYear && localDate.getUTCMonth() === selectedMonth && localDate.getUTCDate() === selectedDay) {
          tideEvents.push({ type: extreme.type === "High" ? "High" : "Low", time: local.timeStr, height: extreme.height, timestamp: extreme.dt * 1000 });
        }
      }
    }
    tideEvents.sort((a,b) => a.timestamp - b.timestamp);
    const tideData = { events: tideEvents, moonPhase: getMoonPhase(date), moonIcon: getMoonIcon(getMoonPhase(date)), tideType, timezone: getIrishTimezone() };
    tideCache.set(cacheKey, { data: tideData, timestamp: now });
    return tideData;
  } catch(e) { console.error(e); return { events: [], moonPhase: getMoonPhase(date), tideType: "Unknown", error: true }; }
}

// REAL WEATHER (includes sunrise/sunset)
async function fetchRealWeather(station, date) {
  try {
    const lat = station.lat, lon = station.lon, dateStr = formatDateForAPI(date);
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=wind_speed_10m,wind_direction_10m,wind_gusts_10m,visibility,rain,cloudcover,temperature_2m,uv_index&daily=sunrise,sunset&timezone=auto&start_date=${dateStr}&end_date=${dateStr}`;
...
async function fetchRealSwellData(station, date) {
  try {
    const lat = station.lat, lon = station.lon, dateStr = formatDateForAPI(date);
    const url = `https://marine-api.open-meteo.com/v1/marine?latitude=${lat}&longitude=${lon}&hourly=wave_height,wave_direction,wave_period&timezone=auto&start_date=${dateStr}&end_date=${dateStr}`;
    const resp = await fetch(url);
...
function isSlackWaterTime(tideEvents, hour, minute) {
  ...
  for (let t of tideEvents) {
    ...
    if (Math.abs(total - tideMin) <= 40) return true;
  }
  return false;
}
...
// EXPORT (full version with all sections)
async function getFormattedExportText() {
  ...
  let coxMode = '';
  for (let r of coxModeRadios) if (r.checked) { coxMode = r.value; break; }
  const partRadios = document.querySelectorAll('input[name="participation"]');
  let participation = "Open to All";
  for (let r of partRadios) if (r.checked) { participation = r.value; break; }
  const diveTypeRadios = document.querySelectorAll('input[name="diveType"]');
  let diveType = "Boat";
  for (let r of diveTypeRadios) if (r.checked) { diveType = r.value; break; }
  ...
  let hourWeather = null, hourSwell = null;
  for (let w of weather) if (parseInt(w.time) === currentHour) hourWeather = w;
  for (let s of swell) if (parseInt(s.time) === currentHour) hourSwell = s;
  ...
  const targetMin = currentHour*60 + currentMinute;
  for (let t of tides.events) {
    const m = parseInt(t.time.split(':')[0])*60 + parseInt(t.time.split(':')[1]);
    if (m <= targetMin) prevTide = t;
    if (m >= targetMin && !nextTide) nextTide = t;
  }
  ...
  let partText = participation;
  if (participation === "Open to All") partText = "Open to All (with appropriate buddy pairs)";
  else if (participation === "Restricted to D2+") partText = "Restricted to D2+ (with appropriate buddy pairs)";
...
// SPINNERS with extended edge values
...
async function updateTides() {
  const tides = await fetchRealTideData(currentStation, currentDate);
  if (tides.error || !tides.events.length) { document.getElementById('tideData').innerHTML = '<div class="tide-event">⚠️ Tide data unavailable for this station/date</div>'; return; }
...
// HOURLY with fixed tide direction
async function updateHourly() {
  ...
  if (!container) return;
  if (!weather.length || weather[0]?.error) { container.innerHTML = '<div style="text-align:center; padding:20px;">⚠️ Weather data unavailable</div>'; return; }

  function getTideDirection(tideEvents, hour) {
    if (!tideEvents.length) return "No Data";
    const tidesMin = tideEvents.map(t => ({ type: t.type, minutes: parseInt(t.time.split(':')[0])*60 + parseInt(t.time.split(':')[1]) }));
    const target = hour*60;
    let prev = null, next = null;
    for (let td of tidesMin) {
      if (td.minutes <= target) prev = td;
      if (td.minutes >= target && !next) next = td;
    }
    const first = tidesMin[0], last = tidesMin[tidesMin.length-1];
...
async function updateDetailed() {
  ...
  let prevTide = null, nextTide = null;
  const target = selectedHour*60 + selectedMinute;
  for (let t of tides.events) {
    const m = parseInt(t.time.split(':')[0])*60 + parseInt(t.time.split(':')[1]);
    if (m <= target) prevTide = t;
    if (m >= target && !nextTide) nextTide = t;
  }
  // Detailed tide direction (same logic)
  let tideDirection = "No Data";
  const tidesMin = tides.events.map(t => ({ type: t.type, minutes: parseInt(t.time.split(':')[0])*60 + parseInt(t.time.split(':')[1]) }));
  const first = tidesMin[0], last = tidesMin[tidesMin.length-1];
  const slack = (m) => Math.abs(m - target) <= 40;
  if (tidesMin.length === 0) tideDirection = "No Data";
  else if (target < first.minutes) tideDirection = slack(first.minutes) ? "Slack Water ⚡" : (first.type === "High" ? "Flooding (Incoming) 🌊⬆️" : "Ebbing (Outgoing) 🌊⬇️");
  else if (target > last.minutes) tideDirection = slack(last.minutes) ? "Slack Water ⚡" : (last.type === "High" ? "Ebbing (Outgoing) 🌊⬇️" : "Flooding (Incoming) 🌊⬆️");
  else if (prevTide && nextTide) {
    const pMin = parseInt(prevTide.time.split(':')[0])*60 + parseInt(prevTide.time.split(':')[1]);
    const nMin = parseInt(nextTide.time.split(':')[0])*60 + parseInt(nextTide.time.split(':')[1]);
    if (Math.abs(target-pMin)<=40 || Math.abs(target-nMin)<=40) tideDirection = "Slack Water ⚡";
    else if (prevTide.type === "High" && nextTide.type === "Low") tideDirection = "Ebbing (Outgoing) 🌊⬇️";
    else if (prevTide.type === "Low" && nextTide.type === "High") tideDirection = "Flooding (Incoming) 🌊⬆️";
  }
  ...
  if (isSlack && tides.events.length) html += `<div class="detail-row" style="background: rgba(47, 255, 238, 0.15); border-radius: 8px; margin-top: 5px; padding: 8px;"><strong>⚡ Slack Water Alert:</strong> Current time is within 40 minutes of a tide change</div>`;
  if ((prevTide || nextTide) && tides.events.length) {
    html += `<div class="detail-row" style="margin-top:12px;"><strong>📊 Relevant tides for this dive:</strong></div>`;
    if (prevTide) {
      const diff = Math.abs(target - (parseInt(prevTide.time.split(':')[0])*60 + parseInt(prevTide.time.split(':')[1])));
    ...
    if (nextTide) {
      const diff = Math.abs((parseInt(nextTide.time.split(':')[0])*60 + parseInt(nextTide.time.split(':')[1])) - target);
  ...
  } else if (!tides.events.length) html += '<div class="detail-row">⚠️ Tide data unavailable for this station/date</div>';
...
// SAVE/LOAD PLAN functions (unchanged but included)
function saveCurrentPlan() { /* same as earlier, uses getFormattedExportText */ }
function deleteSavedPlan(planId) { savedPlans = savedPlans.filter(p => p.id !== planId); localStorage.setItem('divesense_plans', JSON.stringify(savedPlans)); renderSavedPlans(); showNotification('Plan deleted'); }
function loadSavedPlan(plan) { /* same */ }
function renderSavedPlans() { /* same */ }
function escapeHtml(text) { /* same */ }
function showNotification(msg) { /* same */ }
...
// WhatsApp, Email, Copy, Save buttons (full implementations)
...
document.getElementById('savePlanBtn')?.addEventListener('click', function() { saveCurrentPlan(); });
